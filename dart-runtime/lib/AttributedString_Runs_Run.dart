...
class AttributedString_Runs_Run extends SwiftReference {
  ...
  static CreatedRef ffi_new(ffi.Pointer ref, OutCreatedRef exn) =>
      check((exn) => createRef(AttributedString_Runs_Run(ref)));

  /// The range of the portion of the attributed string that this run description represents.
  /// <!-- FishyJoes.export(range) -->
  SwiftRange<AttributedString_Index> get range => GCRef.using(
    this,
    (_thisHandle) => check(
      (exn) => consumeCreatedRef<SwiftRange<AttributedString_Index>>(
        f__iota_get_Foundation_AttributedString_Runs_Run_range(Loader.shared.env, _thisHandle.ptr, exn),
      ),
    ),
  );
  ...
  AttributeContainer get attributes => GCRef.using(
    this,
    (_thisHandle) => check(
      (exn) => consumeCreatedRef<AttributeContainer>(
        f__iota_get_Foundation_AttributedString_Runs_Run_attributes(Loader.shared.env, _thisHandle.ptr, exn),
      ),
    ),
  );
  ...
  int get hashCode => GCRef.using(
    this,
    (_thisHandle) =>
        check((exn) => f__iota_get_Foundation_AttributedString_Runs_Run_hash(Loader.shared.env, _thisHandle.ptr, exn)),
  );
  ...
  bool operator ==(
    ...
    (thisHandle) => GCRef.using(
      other as AttributedString_Runs_Run,
      (otherHandle) => check(
        (exn) => f__iota_Foundation_AttributedString_Runs_Run_equals(
          Loader.shared.env,
          thisHandle.ptr,
          otherHandle.ptr,
          exn,
