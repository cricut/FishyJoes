...
class AttributedString_Index extends SwiftReference implements Comparable<dynamic> {
  ...
  static CreatedRef ffi_new(ffi.Pointer ref, OutCreatedRef exn) =>
      check((exn) => createRef(AttributedString_Index(ref)));

  @override
  int get hashCode => GCRef.using(
    this,
    (_thisHandle) =>
        check((exn) => f__iota_get_Foundation_AttributedString_Index_hash(Loader.shared.env, _thisHandle.ptr, exn)),
  );
  @override
  bool operator ==(
    Object other,
    ...
    (thisHandle) => GCRef.using(
      other as AttributedString_Index,
      (otherHandle) => check(
        (exn) =>
            f__iota_Foundation_AttributedString_Index_equals(Loader.shared.env, thisHandle.ptr, otherHandle.ptr, exn),
      ),
    ),
  );
  ...
  int compareTo(other) {
    return GCRef.using(
      ...
      (thisHandle) => GCRef.using(
        other,
        (otherHandle) => check(
          (exn) => f__iota_Foundation_AttributedString_Index_compare(
            Loader.shared.env,
            thisHandle.ptr,
            otherHandle.ptr,
            exn,
  ...
  static late int Function(Env env, UnownedRef lhs, UnownedRef rhs, OutCreatedRef _exn)
