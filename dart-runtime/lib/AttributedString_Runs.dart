...
class AttributedString_Runs extends SwiftReference with Iterable<AttributedString_Runs_Run> {
  ...
  static CreatedRef ffi_new(ffi.Pointer ref, OutCreatedRef exn) =>
      check((exn) => createRef(AttributedString_Runs(ref)));

  /// The position of the first run in a nonempty attributed string or substring.
  /// <!-- FishyJoes.export(startIndex) -->
  AttributedString_Runs_Index get startIndex => GCRef.using(
    this,
    (_thisHandle) => check(
      (exn) => consumeCreatedRef<AttributedString_Runs_Index>(
        f__iota_get_Foundation_AttributedString_Runs_startIndex(Loader.shared.env, _thisHandle.ptr, exn),
      ),
    ),
  );
  ...
  AttributedString_Runs_Index get endIndex => GCRef.using(
    this,
    (_thisHandle) => check(
      (exn) => consumeCreatedRef<AttributedString_Runs_Index>(
        f__iota_get_Foundation_AttributedString_Runs_endIndex(Loader.shared.env, _thisHandle.ptr, exn),
      ),
    ),
  );
  @override
  int get hashCode => GCRef.using(
    this,
    (_thisHandle) =>
        check((exn) => f__iota_get_Foundation_AttributedString_Runs_hash(Loader.shared.env, _thisHandle.ptr, exn)),
  );
  ...
  AttributedString_Runs_Index indexBefore(
    ...
    (_thisHandle) => GCRef.using(
      index,
      (_indexHandle) => consumeCreatedRef<AttributedString_Runs_Index>(
        check(
          (OutCreatedRef _exn) => f__iota_Foundation_AttributedString_Runs_indexBefore(
            Loader.shared.env,
            _thisHandle.ptr,
            _indexHandle.ptr,
            _exn,
  ...
  AttributedString_Runs_Index indexAfter(
    ...
    (_thisHandle) => GCRef.using(
      ...
      (_indexHandle) => consumeCreatedRef<AttributedString_Runs_Index>(
        check(
          (OutCreatedRef _exn) => f__iota_Foundation_AttributedString_Runs_indexAfter(
            Loader.shared.env,
            _thisHandle.ptr,
            _indexHandle.ptr,
            _exn,
  ...
  AttributedString_Runs_Run elementAtIndex(
    ...
    (_thisHandle) => GCRef.using(
      index,
      (_indexHandle) => consumeCreatedRef<AttributedString_Runs_Run>(
        check(
          (OutCreatedRef _exn) => f__iota_Foundation_AttributedString_Runs_elementAt(
            Loader.shared.env,
            _thisHandle.ptr,
            _indexHandle.ptr,
            _exn,
  ...
  AttributedString_Runs_Run elementAtPosition(
    ...
    (_thisHandle) => GCRef.using(
      ...
      (_indexHandle) => consumeCreatedRef<AttributedString_Runs_Run>(
        check(
          (OutCreatedRef _exn) => f__iota_Foundation_AttributedString_Runs_elementAtPosition(
            Loader.shared.env,
            _thisHandle.ptr,
            _indexHandle.ptr,
            _exn,
  ...
  bool operator ==(
    ...
    (thisHandle) => GCRef.using(
      other as AttributedString_Runs,
      (otherHandle) => check(
        (exn) =>
            f__iota_Foundation_AttributedString_Runs_equals(Loader.shared.env, thisHandle.ptr, otherHandle.ptr, exn),
      ),
    ),
  );
  ...
  static late CreatedRef Function(Env env, UnownedRef _this, UnownedRef index, OutCreatedRef _exn)
...
class AttributedString_Runs_Iterator implements Iterator<AttributedString_Runs_Run> {
  ...
  AttributedString_Runs_Iterator(AttributedString_Runs view) : view = view, index = null {}
