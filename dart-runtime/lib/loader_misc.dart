...
typedef _Foundation_Data_setup<R> =
    R Function(
      Env env,
...
extension LoaderMisc on Loader {
  static int _stringGetLengthMethod(UnownedRef obj, OutCreatedRef exn) =>
      catching(exn, () => peekRef<String>(obj).length) ?? 0;

  static void _stringGetUtf16Method(UnownedRef obj, ffi.Pointer<ffi.Uint16> outUnits, OutCreatedRef exn) =>
      catching(exn, () {
  ...
  static CreatedRef _stringConstructor(ffi.Pointer<ffi.Uint16> codeUnits, int byteLength, OutCreatedRef exn) =>
      catchingRef(exn, () {
        final codeUnitList = codeUnits.asTypedList(byteLength ~/ 2);
        return createRef(String.fromCharCodes(codeUnitList));
      });

  static int _dataGetLengthMethod(UnownedRef obj, OutCreatedRef exn) =>
      catching(exn, () => peekRef<typed_data.Uint8List>(obj).lengthInBytes) ?? 0;

  static void _dataGetBytesMethod(UnownedRef obj, ffi.Pointer<ffi.Uint8> outBytes, OutCreatedRef exn) =>
      catching(exn, () {
        final data = peekRef<typed_data.Uint8List>(obj);
        outBytes.asTypedList(data.lengthInBytes).setAll(0, data.buffer.asUint8List());
      });

  static CreatedRef _dataConstructor(ffi.Pointer<ffi.Uint8> bytes, int length, OutCreatedRef exn) =>
      catchingRef(exn, () => createRef(typed_data.Uint8List.fromList(bytes.asTypedList(length))));
  ...
  static void _setup(Env env) {
    Loader._dylib.lookupFunction<_Swift_String_setup<ffi.Void>, _Swift_String_setup<void>>('Swift_String_setup')(
      env,
    ...
    Loader._dylib.lookupFunction<_Foundation_Data_setup<ffi.Void>, _Foundation_Data_setup<void>>(
    ...
    Loader._dylib.lookupFunction<_Foundation_URL_setup<ffi.Void>, _Foundation_URL_setup<void>>('Foundation_URL_setup')(
      env,
