...
// (old versions of) typescript think node Buffers are ArrayBuffers for some reason, so test that that works anyway
test('ConsumeNodeBuffer', () => {
    const buffer = Buffer.alloc(8, 'na-') as any;
    expect(arrayFrom(TestAPI.Bytes.echoData(buffer)))
        .toEqual([110, 97, 45, 110, 97, 45, 110, 97]);
});

// (old versions of) typescript also think Uint8Arrays are ArrayBuffers
test('ConsumeUint8Array', () => {
    const u8Array = Uint8Array.from([1, 2, 3, 6, 11, 23, 47, 106, 235]) as any;
    expect(arrayFrom(TestAPI.Bytes.echoData(u8Array)))
        .toEqual([1, 2, 3, 6, 11, 23, 47, 106, 235]);
});

test('ConsumeInt8Array', () => {
    const i8Array = Int8Array.from([1, 2, 3, 6, 11, 23, 47, 106, -21]) as any;
    expect(arrayFrom(TestAPI.Bytes.echoData(i8Array)))
        .toEqual([1, 2, 3, 6, 11, 23, 47, 106, 235]);
});

test('ConsumeUint8ClampedArray', () => {
    let u8cArray = Uint8ClampedArray.from([1, 2, 3, 6, 11, 23, 47, 106, 235]) as any;
...
test('ConsumeUint16Array', () => {
    let u16Array = Uint16Array.from([1, 1, 1, 2, 3, 6, 11]) as any;
...
test('ConsumeBigUint64Array', () => {
    let u16Array = BigUint64Array.from([BigInt("0xdecafbadc0ffee")]) as any;
    expect(arrayFrom(TestAPI.Bytes.echoData(u16Array)))
