...
// The browser entry of @cricut/testapi-wasm (TestAPI.browser.js) avoids
// top-level await for bundler compatibility, so its `TestAPI` / `Runtime` exports start as
// `undefined`. Calling `init()` once populates the module's live ES bindings,
// which then propagate to every test file's `import { TestAPI } from 'TestAPI'`.
import { init } from 'TestAPI'
