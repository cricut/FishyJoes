...
// MARK: - NodeInterface/TestAPI.Tree+node.swift

extension TestAPI.Tree: FishyJoesNodeRuntime.NodeMutator {
    public typealias SwiftType = Self
    public static func fromNode(_ value: NAPI.Value, env: NAPI.Env) throws -> Self {
        Self(
            value: try { () -> Swift.Int in
                let fieldValue = try env.getNamedProperty(value, "value")
                return try Swift.Int.fromNode(fieldValue, env: env)
            }(),
            children: try { () -> Array<TestAPI.Tree> in
                let fieldValue = try env.getNamedProperty(value, "children")
                return try ArrayConverter<TestAPI.Tree>.fromNode(fieldValue, env: env)
            }()
        )
    }
    public static func toNode(_ value: Self, env: NAPI.Env) throws -> NAPI.Value {
        let constructor = try NodeClass.constructor(for: "Tree", module: "TestAPI", env: env)
        let args: [NAPI.Value] = [
            try Swift.Int.toNode(value.value, env: env),
            try ArrayConverter<TestAPI.Tree>.toNode(value.children, env: env),
        ]
        return try env.newInstance(constructor, args)
    }
    public static func mutateNode(_ value: Self, this: NAPI.Value, env: NAPI.Env) throws {
    ...
    public static func nodeSetup(env: NAPI.Env, module: NAPI.Value) throws {
        let nodeClass = try NodeClass(
            ...
            properties: [
                (name: "value", .stored(mutable: true), isStatic: false),
                (name: "children", .stored(mutable: true), isStatic: false),
            ],
            constructor: { env, info in
                callbackBody(env, info, name: "Tree_constructor", expectedArgumentCount: 2) { env in
                    let this = try env.this()
                    try env.env.setNamedProperty(this, "value", env.argument(at: 0))
                    try env.env.setNamedProperty(this, "children", env.argument(at: 1))
                    return this
                }
            }
        )
