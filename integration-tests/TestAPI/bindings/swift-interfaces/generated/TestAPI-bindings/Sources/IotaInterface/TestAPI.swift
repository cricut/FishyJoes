...
// MARK: - IotaInterface/TestAPI.Threading+iota-methods.swift

// Generated by FishyJoes for `TestAPI.Threading.proveMainThreadCanPark()`
@_cdecl("__iota_TestAPI_Threading_proveMainThreadCanPark")
public func __iota_TestAPI_Threading_proveMainThreadCanPark(
    envRef: EnvRef,
    _exn: foreignOutExn
) -> foreignObject {
    let env = Env(envRef)
    return env.catching(to: _exn) {
        ...
        return try FutureConverter<Swift.Bool>.toIota(_swiftFuture, env: env)
    }
}
...
@_cdecl("__iota_TestAPI_Threading_proveParallelism")
public func __iota_TestAPI_Threading_proveParallelism(
    envRef: EnvRef,
    _exn: foreignOutExn
) -> foreignObject {
    let env = Env(envRef)
...
@_cdecl("__iota_TestAPI_Threading_runConcurrentJobs")
public func __iota_TestAPI_Threading_runConcurrentJobs(
    envRef: EnvRef,
    jobCount: Swift.Int.CType,
    chunksPerJob: Swift.Int.CType,
    progress: AsyncFunction2Converter<Swift.Int, Swift.Int, FishyJoesCommonRuntime.VoidConverter>.CType,
    _exn: foreignOutExn
) -> foreignObject {
    let env = Env(envRef)
    return env.catching(to: _exn) {
        let jobCount = try Swift.Int.peekIota(jobCount, env: env)
        let chunksPerJob = try Swift.Int.peekIota(chunksPerJob, env: env)
        let progress = try AsyncFunction2Converter<Swift.Int, Swift.Int, FishyJoesCommonRuntime.VoidConverter>.peekIota(progress, env: env)
        let _swiftFuture = Future {
...
// MARK: - IotaInterface/TestAPI.Threading+iota-type.swift

@_cdecl("TestAPI_Threading_setup")
public func TestAPI_Threading_setup(
    envRef: EnvRef
) {
}

extension TestAPI.Threading: FishyJoesIotaRuntime.IotaConverter {
    public static func peekIota(_ value: foreignObject, env: Env) throws -> Self {
        throw UninhabitedTypeCreationError(self)
    }
