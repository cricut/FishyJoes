...
def ensure_loaded() -> None:
    ...
    # PuttingTypesIntoQuestionablePlaces NativeReference subclass: the generated module
    # defines the name four times; only the last (enum) survives module import.  We need
    # the third definition (NativeReference subclass) for setup_reference_type.
    # Use ast to extract just the third ClassDef from the source file.
    import ast as _ast, os as _os, textwrap as _textwrap
    _ptiqp_path = _os.path.join(_os.path.dirname(__file__), "PuttingTypesIntoQuestionablePlaces.py")
    _ptiqp_src = open(_ptiqp_path).read()
    _ptiqp_tree = _ast.parse(_ptiqp_src, _ptiqp_path)
    # Collect all ClassDef nodes named "PuttingTypesIntoQuestionablePlaces".
    _ptiqp_classdefs = [
        node for node in _ptiqp_tree.body
        if isinstance(node, _ast.ClassDef) and node.name == "PuttingTypesIntoQuestionablePlaces"
    ]
    # The third definition (index 2) is the NativeReference subclass.
    _ptiqp_third_def = _ptiqp_classdefs[2]
    # Build a minimal module containing just the required imports + that class.
    _ptiqp_mini_src = (
        "from __future__ import annotations\n"
        "import weakref\n"
        "from cricut_testapi.runtime import NativeReference, ensure_loaded as _ensure_runtime_loaded, get_runtime as _get_runtime, not_implemented as _not_implemented\n"
        + _ast.unparse(_ptiqp_third_def)
    )
    _ptiqp_ns: dict = {}
    exec(compile(_ptiqp_mini_src, _ptiqp_path, "exec"), _ptiqp_ns)
    _Structs_PTIQP_cls = _ptiqp_ns["PuttingTypesIntoQuestionablePlaces"]
    _runtime.setup_reference_type("TestAPI_Structs_PuttingTypesIntoQuestionablePlaces_setup", _Structs_PTIQP_cls)
    ...
    # ClosedRange types (same setup symbol)
    ...
    # Function types — leaf (no async/future dependencies)
    ...
    # AsyncFunction types — leaf first, then composed
    ...
    # FutureConverter types — leaf first, then nested
    ...
    # AsyncFunction types that depend on AsyncFunction0/1/3/4/5/6 (registered above)
    ...
    # Function types wrapping async/future types (depend on types registered above)
