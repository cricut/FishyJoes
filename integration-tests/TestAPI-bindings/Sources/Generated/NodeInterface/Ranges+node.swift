        ...
        let superclass = try NodeClass(
            ...
            properties: [
                ...
                "closedIntRange": (
                    .accessor(
                        getter: { env, info in
                            FishyJoesNodeRuntime.callbackBody(env, info, name: "closedIntRange", expectedArgumentCount: 0) { env in
                                try ClosedRangeConverter<Int>.toNode(Ranges.closedIntRange, env: env.env)
                            }
                        },
                        setter: nil
                    ),
