...
extension URLs: FishyJoesNodeRuntime.NodeConverter {
    public static func fromNode(_ value: NAPI.Value, env: NAPI.Env) throws -> Self {
        fatalError("invalid enum for URLs")
    }
                    ...
                    .accessor(
                        getter: { env, info in
                            FishyJoesNodeRuntime.callbackBody(env, info, name: "simple", expectedArgumentCount: 0) { env in
                                try Foundation.URL.toNode(URLs.simple, env: env.env)
                            }
                        },
                        setter: nil
                    ),
                    ...
                    .accessor(
                        getter: { env, info in
                            FishyJoesNodeRuntime.callbackBody(env, info, name: "remoteFile", expectedArgumentCount: 0) { env in
                                try Foundation.URL.toNode(URLs.remoteFile, env: env.env)
                            }
                        },
                        setter: nil
                    ),
                    ...
                    .accessor(
                        getter: { env, info in
                            FishyJoesNodeRuntime.callbackBody(env, info, name: "localFile", expectedArgumentCount: 0) { env in
                                try Foundation.URL.toNode(URLs.localFile, env: env.env)
                            }
                        },
                        setter: nil
                    ),
