...
@_cdecl("__cs_AttributedStringASDF_Runs_equals")
public func AttributedStringASDF_Runs_cSharpEquals(lhs: csObject, rhs: csObject, exn: csOutExn) -> Bool.CType {
    Env.catching(to: exn) {
        return try Bool.toCSharp(AttributedStringASDF.Runs.peekCSharp(lhs) == AttributedStringASDF.Runs.peekCSharp(rhs))
    }
}
@_cdecl("__cs_AttributedStringASDF_Runs_hash")
public func AttributedStringASDF_Runs_cSharpHash(this: csObject, exn: csOutExn) -> Int32.CType {
    Env.catching(to: exn) {
        try Int32.toCSharp(
            Int32(truncatingIfNeeded: AttributedStringASDF.Runs.peekCSharp(this).hashValue)
        )
    }
}
