...
extension TestAPI.MutatingCounter: JavaMutator {
    ...
    private static var _java_count_id: jfieldID!
    private static var _constructorMethodID: jmethodID!
    public static func fromJava(_ value: jobject?, env: Env) throws -> Self {
        Self(
            count: try Swift.Int.fromJava(
                env.GetLongField(value, Self._java_count_id),
                env: env
            )
        )
    }
    public static func toJava(_ value: Self, env: Env) throws -> jobject? {
        try env.NewObject(
            ...
            jvalue(Swift.Int.toJava(value.count, env: env))
        )
    }
    ...
    public static func mutateJava<R>(_ this: jobject?, env: Env, body: (inout Self) throws -> R) throws -> R {
        ...
        try env.SetLongField(
            this, Self._java_count_id,
            Swift.Int.toJava(mutatingSelf.count, env: env)
        )
        return result
    }
