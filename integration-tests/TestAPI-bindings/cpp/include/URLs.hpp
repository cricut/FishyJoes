...
namespace TestAPI {
    ...
    class URLs {
        ...
        public:
        template <typename T>
        URLs(const T& caseObj): _variant(caseObj) {}
        
        template <typename T>
        URLs& operator=(const T& rhs) {
            _variant = rhs;
            return *this;
        }
        ...
        private:
        /// Create new URLs (only to be used by FishyJoes internally. Look for static methods for initialization or other public constructors.)
        URLs(const std::variant<> &_variant);
        
        /*  Methods  */
        public:
        /// <!-- FishyJoes.export(echo) -->
        static URL echo(const URL &url);
        
        /// <!-- FishyJoes.export(simple) -->
        static URL getSimple();
        
        /// <!-- FishyJoes.export(remoteFile) -->
        static URL getRemoteFile();
        
        /// <!-- FishyJoes.export(localFile) -->
        static URL getLocalFile();
