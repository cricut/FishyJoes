...
namespace TestAPI {
    ...
    std::unordered_map<std::string, std::string> AttributedStringASDF::getAttributes() const {
        return FishyJoesInternal::Packer::unpack<std::unordered_map<std::string, std::string>>(FishyJoesInternal::CBindings::FJInternalBinding_TestAPI_AttributedStringASDF_getAttributes(FishyJoesInternal::Packer::pack(*this).ptr()));
    }
}
