...
namespace Cricut.TestAPI {
    ...
    public class AttributedSubstring : SwiftReference {
        ...
        public System.Collections.Generic.IDictionary<string, string> GetAttributes() {
            using var thisHandle = new GCRef(this);
            return Check((out CreatedRef exn) =>
                __cs_get_AttributedSubstringASDF_Attributes(thisHandle.ptr, out exn).Consume<System.Collections.Generic.IDictionary<string, string>>()
            );
        }

        [DllImport("TestAPI-c-sharp", ExactSpelling = true, CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Unicode)]
        private static extern CreatedRef __cs_get_AttributedSubstringASDF_Attributes(UnownedRef self, out CreatedRef exn);
