...
namespace Cricut.FishyJoesRuntime {
    ...
    public class SwiftReference : IDisposable {
        ...
        public override string ToString() {
            return Check((out CreatedRef exn) => FishyJoesRuntime_AnyBox_toString(Loader.env, new UnownedRef(reference), out exn)).Consume<string>();
        }
        ...
        public void Dispose() {
            ...
            if (this.reference != IntPtr.Zero) {
                Check((out CreatedRef exn) => SwiftReference.FishyJoesRuntime_AnyBox_releaseRef(Loader.env, new ConsumedRef(reference), out exn));
            }
            this.reference = IntPtr.Zero;
        }

        [DllImport("FishyJoesIotaRuntime", ExactSpelling = true, CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Unicode)]
        private static extern void FishyJoesRuntime_AnyBox_releaseRef(IntPtr envRef, ConsumedRef swiftReference, out CreatedRef exn);

        [DllImport("FishyJoesIotaRuntime", ExactSpelling = true, CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Unicode)]
        private static extern CreatedRef FishyJoesRuntime_AnyBox_toString(IntPtr envRef, UnownedRef swiftReference, out CreatedRef exn);
    ...
    public partial class Loader {
        ...
        [DllImport("FishyJoesIotaRuntime", ExactSpelling = true, CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Unicode)]
        static extern void FishyJoesRuntime_AnyBox_setup(IntPtr envRef, AnyBoxRefGetter refGetter);
