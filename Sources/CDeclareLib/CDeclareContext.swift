...
public class CDeclareContext {
    ...
    func swiftFragment(_ name: String) -> SourceFragment {
        fileHeaders[name, default: []].insert("@_exported import CTypes\nimport \(module)")
    ...
    func translate(method: SourceryMethod) -> [SourceFragment] {
        ...
        if let selfType = method.definedInTypeName?.better {
            ...
            if method.isStatic {
                ...
                let selfParameter = isMutating ? .unsafeMutablePointer(selfType) : selfType
                let selfPointerFormals = resolve(type: selfParameter).asFormals(label: "self", name: "self")
                formals.append(contentsOf: selfPointerFormals.formals)
                selfExpression = selfPointerFormals.expression
            }
        } else {
        ...
        var omitParameters = ((method.annotations["cdecl.omitParameters"] as? String).map { Set($0.components(separatedBy: " ")) }) ?? []
        let parameters = method.parameters.compactMap { parameter -> (formals: [String], asArgument: String)? in
            ...
            let translated = resolve(type: parameter.typeName.better)
                .asFormals(label: parameter.argumentLabel ?? "_", name: parameter.name)
            return (
                formals: translated.formals,
                asArgument: (parameter.argumentLabel.map { "\($0): " } ?? "") + translated.expression
            )
        }
        precondition(omitParameters.isEmpty, "Can't find parameters \(omitParameters) to omit")

        formals.append(contentsOf: parameters.flatMap(\.formals))
        ...
        fragment.output("// Generated by sourcery:cdecl for `\(containingNamespace).\(method.name)`")
        ...
        fragment.outputBlock("\(translatedReturn.globalCName == "Swift.Void" ? "" : " -> \(translatedReturn.globalCName)") {") {
            ...
            fragment.outputBlock("\(selfExpression)\(callName)(", newLineTerminated: false) {
                fragment.outputMap(parameters, separator: ",") { $0.asArgument }
            }
            fragment.output(translatedReturn.asCAccessor)
        }
        return [fragment]
    }
    ...
    func translateGetter(for variable: Variable) -> [SourceFragment] {
        ...
        if let selfType = variable.definedInTypeName?.better {
            ...
            if variable.isStatic {
                ...
                formals.append(contentsOf: translated.formals)
