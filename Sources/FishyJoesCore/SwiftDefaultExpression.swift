...
extension SwiftDefaultExpression {
    ...
    /// Whether a default value is an integer limit whose value depends on the platform word width
    /// (`Int.min`, `Int.max`, `UInt.max`, or the equivalent implicit `.min`/`.max` on an `Int`/`UInt`
    /// parameter). Emitting these as 64-bit literals breaks targets where the bridged type can be
    /// 32-bit (e.g. C#'s `nint`), so they are evaluated in Swift via an iota default provider instead.
    static func isPlatformWidthIntegerLimit(_ value: String, parameterTypeName: String) -> Bool {
        guard let expression = parse(value) else {
            return false
        }

        let components: [String]
        switch expression {
        case let .memberAccess(parts):
            components = parts
        case let .implicitMember(member):
            components = [parameterTypeName, member]
        default:
            return false
        }
