...
final class PythonTranslator: Translator {
    ...
    func setupFragments(context: FishyJoesContext, generatedTypes: [BetterType]) -> [SourceFragment] {
        ...
        nativeStubFragment.output("class SwiftRange:")
        nativeStubFragment.indent {
            nativeStubFragment.output("\"\"\"A Swift `Range<Int>`; `upper_bound` is excluded.\"\"\"")
            nativeStubFragment.output("lower_bound: int")
            nativeStubFragment.output("upper_bound: int")
        }
        nativeStubFragment.blankLine()
        nativeStubFragment.output("@dataclass(frozen=True)")
        nativeStubFragment.output("class SwiftClosedRange:")
        nativeStubFragment.indent {
            nativeStubFragment.output("\"\"\"A Swift `ClosedRange<Int>`; `upper_bound` is included.\"\"\"")
            nativeStubFragment.output("lower_bound: int")
            nativeStubFragment.output("upper_bound: int")
        }
        nativeStubFragment.blankLine()
        nativeStubFragment.output("@dataclass(frozen=True)")
        nativeStubFragment.output("class ResultSuccess:")
        nativeStubFragment.indent {
            nativeStubFragment.output("\"\"\"The success case of a Swift `Result`, carrying the value.\"\"\"")
            nativeStubFragment.output("value: object")
            nativeStubFragment.output("def get_or_none(self) -> object: ...")
            nativeStubFragment.output("def exception_or_none(self) -> None: ...")
        }
        nativeStubFragment.blankLine()
        nativeStubFragment.output("@dataclass(frozen=True)")
        nativeStubFragment.output("class ResultFailure:")
        nativeStubFragment.indent {
            nativeStubFragment.output("\"\"\"The failure case of a Swift `Result`, carrying the error.\"\"\"")
            nativeStubFragment.output("error: object")
            nativeStubFragment.output("def get_or_none(self) -> None: ...")
            nativeStubFragment.output("def exception_or_none(self) -> object: ...")
        }
