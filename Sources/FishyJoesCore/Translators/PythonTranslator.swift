...
final class PythonTranslator: Translator {
    ...
    static func emitAttributedStringFamilyStubs(into fragment: SourceFragment) {
        ...
        fragment.indent {
            fragment.output("def __init__(self, string: str, attributes: Runtime_AttributeContainer | None = ...) -> None: ...")
            fragment.output("@property")
    ...
    private func classStubFragment(_ pythonClass: PythonClass) -> SourceFragment {
        ...
        let stubAnnotations = pythonStubAnnotationStrings(for: pythonClass, shadowedBuiltinTypes: shadowedBuiltinTypes)
    ...
    private func pythonStubAnnotationStrings(for pythonClass: PythonClass, shadowedBuiltinTypes: Set<String>) -> [String] {
        var annotations: [String] = []
        annotations += pythonClass.storedFields.map { pythonTypeAnnotation($0.pythonType, shadowedBuiltinTypes: shadowedBuiltinTypes) }
        for enumCase in pythonClass.enumCases {
            annotations += enumCase.values.map { pythonTypeAnnotation($0.pythonType, shadowedBuiltinTypes: shadowedBuiltinTypes) }
        }
        for field in pythonClass.fields {
            annotations.append(pythonTypeAnnotation(field.pythonReturnType, shadowedBuiltinTypes: shadowedBuiltinTypes))
        }
        for method in pythonClass.methods {
            annotations.append(pythonTypeAnnotation(method.pythonReturnType, shadowedBuiltinTypes: shadowedBuiltinTypes))
            annotations += method.parameters.map { pythonTypeAnnotation($0.pythonType, shadowedBuiltinTypes: shadowedBuiltinTypes) }
        }
        return annotations
    }
