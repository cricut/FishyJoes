...
final class KotlinTranslator: Translator {
    ...
    func translate(variable: SourceryVariablePlus, context: FishyJoesContext, type: Type) -> [SourceFragment] {
        ...
        fragment.outputBlock(" -> \(cType) = { \(formals.map(\.name).joined(separator: ", ")) in", closeWith: "}") {
            fragment.outputBlock("FishyJoesJavaRuntime.callbackBody(_javaEnv) { _javaEnv in", closeWith: "}") {
                let value = "\(selfExpression).\(variable.sourceryVariable.name)"
        ...
        if variable.sourceryVariable.isPubliclyWritable {
            ...
            fragment.outputBlock(" -> Void = { \(formals.map(\.name).joined(separator: ", ")) in", closeWith: "}") {
                fragment.outputBlock("FishyJoesJavaRuntime.callbackBody(_javaEnv) { _javaEnv in", closeWith: "}") {
                    if variable.sourceryVariable.isStatic {
                        fragment.output("\(selfExpression).\(variable.sourceryVariable.name) = try \(converterName).fromJava(newValue, env: _javaEnv)")
                    } else {
                        fragment.outputBlock("try \(sourceTypeName).mutateJava(_javaThis, env: _javaEnv) { value in", closeWith: "}") {
                            fragment.output("value.\(variable.sourceryVariable.name) = try \(converterName).fromJava(newValue, env: _javaEnv)")
                        }
                    }
                }
            }
