...
final class KotlinTranslator: Translator {
    ...
    func translate(method: Method, context: FishyJoesContext, isProtocol: Bool, typeName: String) -> [SourceFragment] {
        ...
        let destinationRootFilename: String
        ...
        if let selfType = method.definedIn {
            ...
            destinationRootFilename = context.module.name
        ...
        let fragment = context.swiftFragment(
            "JavaInterface/\(destinationRootFilename)+java-methods.swift",
            additionalImports: ["Foundation", "FishyJoesJavaRuntime", "\(context.module.name)_CommonInterface"]
        )
        fragment.output("// Generated by FishyJoes for `\(destinationRootFilename).\(method.name)`")
        var jniSignature = ""
        if isProtocol,
           method.isInExtension,
        ...
        var cMethod = "java_\(destinationRootFilename)"
        if isProtocol,
           method.isInExtension {
            cMethod += "__default"
        }
        ...
        allMethods[destinationRootFilename, default: []].append((javaName, jniSignature, cMethod, isProtocol && method.isInExtension))
        ...
        fragment.outputBlock(" -> \(returnSignature) = { \(formals.map(\.name).joined(separator: ", ")) in", closeWith: "}") {
            fragment.outputBlock("FishyJoesJavaRuntime.callbackBody(_javaEnv) { _javaEnv in", closeWith: "}") {
                ...
                if method.isAsync {
                    ...
                    if method.isMutating {
                        mutateBlock = { body in
                            fragment.outputBlock("return try \(destinationRootFilename).mutateJava(_javaThis, env: _javaEnv) { mutatingSelf in", closeWith: "}") {
                                body()
                            }
                        }
                        selfExpression = "mutatingSelf"
    ...
    func translate(variable: Variable, context: FishyJoesContext) -> [SourceFragment] {
        ...
        let cGetName = "java_get_\(destinationRootFilename)_\(exportAnnotation.name)".replacingOccurrences(of: ".", with: "_")
        let cSetName = "java_set_\(destinationRootFilename)_\(exportAnnotation.name)".replacingOccurrences(of: ".", with: "_")
        ...
        allMethods[destinationRootFilename, default: []].append((jvmGetName, "()" + jniSignature, cGetName, false))
        fragment.output("// Generated by FishyJoes for `\(destinationRootFilename).\(variable.name)`")
        ...
        if variable.isPubliclyWritable {
            ...
            allMethods[destinationRootFilename, default: []].append((jvmSetName, "(\(jniSignature))V", cSetName, false))
            if let deprecation = variable.deprecation {
                fragment.output(deprecation.swiftDeprecation)
            }
            ...
            fragment.outputBlock(" -> Void = { \(formals.map(\.name).joined(separator: ", ")) in", closeWith: "}") {
                fragment.outputBlock("FishyJoesJavaRuntime.callbackBody(_javaEnv) { _javaEnv in", closeWith: "}") {
                    if variable.isStatic {
                        ...
                        fragment.outputBlock("try \(destinationRootFilename).mutateJava(_javaThis, env: _javaEnv) { value in", closeWith: "}") {
