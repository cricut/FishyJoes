...
final class IotaTranslator: Translator {
    ...
    func translate(method: Method, context: FishyJoesContext, betterType: BetterType) -> [SourceFragment] {
        ...
        outputDefaultProviders(for: method, containingNamespace: containingNamespace, context: context, to: fragment)

        return [fragment]
    }

    /// Default values like `Int.max` depend on the platform word width, so they can't be embedded
    /// as constants in bindings whose corresponding type may be 32-bit (e.g. C#'s `nint`). Expose
    /// them as functions that evaluate the original Swift expression on the running platform.
    private func outputDefaultProviders(
        for method: Method,
        containingNamespace: String,
        context: FishyJoesContext,
        to fragment: SourceFragment
    ) {
        let exportAnnotation = method.exportAnnotation
        for parameter in method.parameters {
            ...
            let resolved = context.resolve(type: parameter.type, generics: exportAnnotation.genericOverrides)
            guard resolved is TranslatedPrimitive || resolved is TranslatedUnsignedPrimitive,
            ...
            let cProvider = "__iota_\(containingNamespace)_\(exportAnnotation.name)_default_\(parameter.name)".mangled
            fragment.blankLine()
            fragment.output("// Generated by FishyJoes for the default value of `\(parameter.name)` in `\(containingNamespace).\(method.name)`")
            fragment.output("@_cdecl(\"\(cProvider)\")")
            fragment.outputBlock("public func \(cProvider)(", newLineTerminated: false) {
                fragment.output("envRef: EnvRef,")
                fragment.output("_exn: foreignOutExn")
            }
            fragment.outputBlock(" -> \(resolved.converterType.name).CType {") {
                fragment.output("let env = Env(envRef)")
                fragment.outputBlock("return env.catching(to: _exn) {") {
                    fragment.output("let _defaultValue: \(resolved.sourceType.name) = \(defaultValue)")
                    fragment.output("return try \(resolved.converterType.name).toIota(_defaultValue, env: env)")
                }
            }
        }
    }
    ...
    func translate(field: Field, context: FishyJoesContext, type: SourceryType) -> [SourceFragment] {
        ...
        let iotaGetName = field.isDefaultImplementation ? "__iota__default_\(sourceTypeName)_\(iotaName)".mangled : "__iota_get_\(sourceTypeName)_\(iotaName)".mangled
        ...
        let resolvedVariable = context.resolve(type: field.type)
        ...
        fragment.outputBlock(" -> \(cType) {") {
            fragment.output("let env = Env(envRef)")
            fragment.outputBlock("return env.catching(to: _exn) {") {
                fragment.output("try \(resolvedVariable.converterType.name).toIota(\(selfExpression).\(field.name), env: env)")
            }
        }
        ...
        if field.isPubliclyWritable {
            ...
            fragment.outputBlock(" {") {
                ...
                fragment.outputBlock("env.catching(to: _exn) {") {
                    if field.isStatic {
                        fragment.output("\(selfExpression).\(field.name) = try \(resolvedVariable.converterType.name).peekIota(newValue, env: env)")
                    } else {
                        fragment.outputBlock("try \(converterTypeName).withMutatingIota(_iotaThis, env: env) { value in", closeWith: "}") {
                            fragment.output("value.\(field.name) = try \(resolvedVariable.converterType.name).peekIota(newValue, env: env)")
                        }
                    }
                }
            }
    ...
    func setupFragments(context: FishyJoesContext, generatedTypes: [BetterType]) -> [SourceFragment] {
        ...
        let exportFragment = context.swiftFragment("IotaInterface/@_exported.swift", withDedicatedFile: true)
