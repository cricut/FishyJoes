...
struct TranslatedStruct: TranslatedType {
    ...
    init(context: FishyJoesContext, type: Type) {
        ...
        self.computedVariables = SourceryVariablePlus.variables(type: type).compactMap { Field($0.sourceryVariable, type: type, isDefaultImplementation: $0.isDefaultImplementation) }
            ...
            fragment.outputBlock("public static func nodeSetup(env: NAPI.Env, module: NAPI.Value) throws {") {
                ...
                fragment.outputBlock("let nodeClass = try NodeClass(") {
                    ...
                    fragment.outputBlock("properties: [", closeWith: "],") {
                        ...
                        hasProperties ||= context.nodeTranslator.outputProperties(computedVariables: normalProperties, context: context, fragment: fragment, converterName: converterType.name)
                        hasProperties ||= context.nodeTranslator.outputProperties(computedVariables: defaultProperties, context: context, fragment: fragment, converterName: converterType.name)
