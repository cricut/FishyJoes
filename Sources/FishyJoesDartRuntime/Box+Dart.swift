...
extension Box {
    public static func peekDart(_ value: foreignObject, env: Env) throws -> Box<T> {
        try Box(inner: AnyBox.peekDart(value, env: env))
    }
}
...
public func toString(envRef: EnvRef, ptr: UnsafeMutableRawPointer?, _ exn: foreignOutExn) -> foreignObject {
    ...
    return env.catching(to: exn) {
        try String.toDart("\(AnyBox.takeUnretainedOpaque(try Env.unwrap(ptr)).value)", env: env)
    }
}
...
extension AnyBox {
    ...
    public static func peekDart(_ value: foreignObject, env: Env) throws -> AnyBox {
